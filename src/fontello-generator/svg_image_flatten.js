...
function processTree(node, ignoredTags, ignoredAttrs, parentTransforms, path) {
	var guaranteed = true;
	pkg.each(node.childNodes, function (item) {
		...
		pkg.each(item.attributes, function (item) {
...
function getCoordinates(svg) {
	...
	var attr = {};
	pkg.forEach(['x', 'y', 'width', 'height'], function (key) {
...
export default function convert(sourceXml) {
	...
	result.ignoredTags = pkg.keys(processed.ignoredTags);
	result.ignoredAttrs = pkg.keys(processed.ignoredAttrs);
	return result;
}
